...
public class ItemStages {
    ...
    public static String getStage (ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            ...
            for (final Tuple<ItemStack, String> entry : SORTED_ITEM_STAGES.get(stack.getItem())) {
                ...
                if (StageCompare.INSTANCE.isValid(stack.getItem().getDefaultInstance(), entry.getFirst())) {
    ...
    public void onPlayerDig (BreakSpeed event) {
        ...
        if (!ConfigurationHandler.allowInteractRestricted && !event.getEntityPlayer().isCreative()) {
            ...
            ItemStack heldItem = event.getEntityPlayer().getHeldItemMainhand().getItem().getDefaultInstance();
    ...
    public void onPlayerAttack (AttackEntityEvent event) {
        ...
        if (!ConfigurationHandler.allowInteractRestricted && !event.getEntityPlayer().isCreative()) {
            ...
            final String stage = getEnchantStage(event.getEntityPlayer().getHeldItemMainhand().getItem().getDefaultInstance());
    ...
    public void onPlayerInteract (PlayerInteractEvent event) {
        ...
        if (event.isCancelable() && !ConfigurationHandler.allowInteractRestricted && !event.getEntityPlayer().isCreative()) {
            ...
            final String stage = getStage(event.getItemStack().getItem().getDefaultInstance());
    ...
    public void onLivingUpdate (LivingUpdateEvent event) {
        ...
        if (event.getEntity() instanceof EntityPlayer && !event.getEntityLiving().world.isRemote) {
            ...
            for (final EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
                ...
                final ItemStack stack = player.getItemStackFromSlot(slot).getItem().getDefaultInstance();
                final ItemStack stack1 = player.getItemStackFromSlot(slot);
